...
def _melt_regressors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    long["purity_type_norm"] = (
        long["purity_type_raw"].str.strip().str.replace(r"\s+", " ", regex=True).str.casefold()
    )
    mapping = (
...
def _find_geo_cols(df: pd.DataFrame) -> Tuple[Optional[str], Optional[str]]:
    ...
    return _find_col(["latitude", "lat"], cols), _find_col(["longitude", "lon", "lng"], cols)


def _get_spectrum_cols(df: pd.DataFrame):
    """Retourne (colonnes_spectrum_*, série_wavelengths)."""
    spec_cols = [c for c in df.columns if isinstance(c, str) and c.startswith("spectrum_")]
    if not spec_cols:
...
def _find_weight_col(df: pd.DataFrame) -> Optional[str]:
    ...
    return _find_col(
        ["net_weight", "weight_net", "netweight", "poids_net", "poids (net)", "poids_net_g", "weight", "poids"],
        df.columns.tolist(),
    )
...
def render_cluster_detail_section(payload: Optional[dict] = None) -> None:
    ...
    if not payload:
        st.info("Générez d’abord des clusters dans l’onglet Profilage (aucun détail disponible).")
        return
    ...
    sizes_map = cluster_df_full.groupby("cluster")["label"].count().rename("taille").to_dict()
    ...
    sample_list = cluster_df.loc[cluster_df["cluster"] == int(chosen_cluster), "label"].astype(str).tolist()
    if not sample_list:
    ...
    # Onglets
    tab_sum, tab_purity, tab_time, tab_map = st.tabs(["Résumé", "Pureté", "Temporalité", "Carte"])

    # ----- Résumé
    with tab_sum:
        st.markdown(f"**Cluster {chosen_cluster}** — {_label_cluster(chosen_cluster)}")

        if (not subset_raw.empty) and date_col_raw and (date_col_raw in subset_raw.columns):
        ...
        with st.expander("Liste des échantillons du cluster", expanded=True):
            base_samples = pd.DataFrame({sample_col: pd.Series(sample_list, dtype=str)}).drop_duplicates()

            # Date (priorité df_raw)
            if (df_raw is not None) and date_col_raw and (date_col_raw in df_raw.columns):
                ...
                last_date_raw["Date"] = pd.to_datetime(last_date_raw["Date"]).dt.strftime("%d/%m/%Y")
            else:
                last_date_raw = pd.DataFrame(columns=[sample_col, "Date"])

            # Pureté moyenne (regressors_*)
            pure_long_all = _melt_regressors(df_raw) if df_raw is not None else pd.DataFrame()
            if not pure_long_all.empty and (sample_col in pure_long_all.columns):
                ...
                pure_sub_tbl = pure_long_all[pure_long_all[sample_col].isin(base_samples[sample_col])].copy()
                ...
                if "seizure_date" in pure_sub_tbl.columns:
                    pure_sub_tbl["seizure_date"] = _parse_dates_series(pure_sub_tbl["seizure_date"])
                    last_date_pur = (
                    ...
                    last_date_pur["Date"] = pd.to_datetime(last_date_pur["Date"]).dt.strftime("%d/%m/%Y")
                else:
            ...
            tbl = tbl.merge(last_date_raw if not last_date_raw.empty else last_date_pur, on=sample_col, how="left")
            tbl = tbl.merge(agg_weight, on=sample_col, how="left")
            tbl = tbl.rename(columns={sample_col: "sample"})
            tbl = tbl[[c for c in ["sample", "Pureté moyenne", "Date", "Poids net"] if c in tbl.columns]]
    ...
    with tab_purity:
        ...
        if pure_long_all.empty:
            ...
            pure_sub = (
                pure_long_all[pure_long_all[sample_col].astype(str).isin(sample_list)].copy()
            ...
            if pure_sub.empty:
                ...
                subs_avail = (
                    ...
                    if "substance" in pure_sub.columns else []
                )
                chosen_sub_p = st.selectbox("Substance", subs_avail, key="cluster_detail_subsel") if subs_avail else None
                if chosen_sub_p:
                    pure_sub = pure_sub[pure_sub["substance"].astype(str) == chosen_sub_p]

                types_avail = (
                    sorted(pure_sub["purity_type_display"].dropna().unique().tolist())
                    if "purity_type_display" in pure_sub.columns else []
                )
                chosen_type_p = (
                    st.selectbox("Type de pureté", types_avail, key="cluster_detail_typesel")
                    if types_avail else None
                )

                date_col_present = "seizure_date" in pure_sub.columns
                if date_col_present:
                    pure_sub["seizure_date"] = _parse_dates_series(pure_sub["seizure_date"])
                ...
                key_sample = sample_col if (sample_col in base_p.columns) else ("sample_name" if "sample_name" in base_p.columns else None)
                ...
                if base_p.empty:
                    ...
                    agg_mode = st.radio("Agrégation", ["Tous", "Année", "Mois", "Semaines"], horizontal=True, index=0, key="cluster_detail_aggmode")

                    def label_bucket(dt: pd.Timestamp, mode: str) -> str:
                        if mode == "Année": return dt.strftime("%Y")
                        if mode == "Mois": return dt.strftime("%Y-%m")
                        if mode == "Semaines":
                    ...
                    if agg_mode == "Tous" or "seizure_date" not in base_p.columns:
                        ...
                        base_p["bucket"] = base_p["seizure_date"].apply(lambda x: label_bucket(x, agg_mode))

                    grouped_purity = (
                        base_p.groupby(["bucket", key_sample], as_index=False)["purity_value"]
                        .mean().rename(columns={"purity_value": "purity_mean"})
                    )
                    ...
                    if agg_mode == "Tous":
                        ...
                        def bucket_key(b):
                            if agg_mode == "Année":  return pd.Timestamp(int(b), 1, 1)
                            if agg_mode == "Mois":
                                y, m = b.split("-");  return pd.Timestamp(int(y), int(m), 1)
                            if agg_mode == "Semaines":
                                y, w = b.split("-W"); return pd.to_datetime(f"{y}-W{w}-1", format="%G-W%V-%u")
                    ...
                    st.altair_chart(
                        ...
                        .encode(
                            x=alt.X("bucket:N", sort=bucket_order, title=("Période" if agg_mode != "Tous" else "")),
                            y=alt.Y("purity_mean:Q", title=f"Pureté (moy./sample){' — '+chosen_type_p if chosen_type_p else ''}"),
                            tooltip=[alt.Tooltip("bucket:N", title="Période"),
    ...
    with tab_time:
        ...
        if (subset_raw.empty) or (not date_col_raw) or (date_col_raw not in subset_raw.columns):
            ...
            temp_last = (
                temp.dropna(subset=[date_col_raw]).sort_values([sample_col, date_col_raw])
                .groupby(sample_col, as_index=False).tail(1).rename(columns={date_col_raw: "date"})
            )
            if temp_last.empty:
                ...
                st.altair_chart(
                    ...
                    .encode(
                        ...
                        y=alt.Y("_stack:Q", axis=alt.Axis(title=None, ticks=False, labels=False), scale=y_scale),
                        color=color_enc,
                        tooltip=[
                            alt.Tooltip("sample:N", title="Sample"),
                            alt.Tooltip("date_day:T", title="Date", format="%d/%m/%Y"),
                            *([alt.Tooltip(f"{namex_col_raw}:N", title="Substance")] if (namex_col_raw and namex_col_raw in plot_df.columns) else []),
                        ],
                    )
    ...
    with tab_map:
        ...
        lat_col, lon_col = (_find_geo_cols(subset_raw) if not subset_raw.empty else (None, None))
        if subset_raw.empty or not lat_col or not lon_col:
            ...
            if gmap.empty:
                ...
                if date_col_raw and (date_col_raw in gmap.columns):
                    gmap["date_str"] = pd.to_datetime(gmap[date_col_raw], errors="coerce").dt.strftime("%d/%m/%Y")
                    tooltip_fields.append(("Date", "date_str"))
                tooltip_html = "<br/>".join([f"<b>{lbl}:</b> {{{col}}}" for lbl, col in tooltip_fields]) if tooltip_fields else ""
                ...
                try:
                    ...
                    view_state = pdk.ViewState(float(vp["latitude"]), float(vp["longitude"]), float(vp["zoom"]), min_zoom=2, max_zoom=16)
                except Exception:
                ...
                deck = deck_with_fallback([layer], view_state, {"html": tooltip_html} if tooltip_html else None)
...
with st.sidebar:
    ...
    page = st.radio(
        "Aller à…",
        ["Accueil", "Données", "Statistiques", "Carte", "Profilage", "Valises marocaines"],
    )
    st.markdown("---")
...
if page == "Accueil":
    ...
    try:
        dt_modif = datetime.fromtimestamp(os.path.getmtime(__file__)).strftime("%d/%m/%Y à %H:%M:%S")
...
# ------------------------ Données ---------------------------
    ...
    if uploaded:
        ...
        query = st.text_input("Contient (toutes colonnes) :", placeholder="ex: client, REF-2025, etc.")
        if query and query.strip():
            q = query.strip()
            filtered = df[df.apply(lambda r: r.astype(str).str.contains(q, case=False, na=False).any(), axis=1)]
        else:
            filtered = df
...
# ---------------------- Statistiques ------------------------
    ...
    if df is None:
        st.warning("⚠️ Aucun fichier en mémoire — importe d’abord un fichier dans l’onglet Données.")
        st.stop()
    ...
    kpi2 = df[seizure_col].dropna().nunique() if seizure_col and seizure_col in df.columns else 0
    ...
    if namex_col and sample_col:
        base_unique = df.dropna(subset=[sample_col]).drop_duplicates(subset=[sample_col])
        counts = (
            base_unique.dropna(subset=[namex_col])
            .groupby(namex_col).size().reset_index(name="count")
        ...
        if counts.empty:
            ...
            cmap = _ensure_color_map(subs_order, st.session_state.get("substance_colors", {}), PALETTE)
            st.session_state["substance_colors"] = cmap

            col_ctrl1, col_ctrl2 = st.columns([1, 1])
            with col_ctrl1:
                chart_type = st.radio("Type de graphique", ["Histogramme", "Circulaire"], horizontal=True, index=0, key="subs_chart_type")
            with col_ctrl2:
                measure = st.radio("Mesure", ["Nombre", "Pourcentage (%)"], horizontal=True, index=0, key="subs_measure")
            ...
            if chart_type == "Histogramme":
                ...
                y_title = "Nombre d'échantillons" if measure == "Nombre" else "Proportion (%)"
                st.altair_chart(
                    alt.Chart(counts)
                    ...
                    .encode(
                        ...
                        color=alt.Color(
                            f"{namex_col}:N", legend=None,
                            scale=alt.Scale(domain=subs_order, range=[cmap[s] for s in subs_order]),
                        ),
                        tooltip=[
    ...
    display_mode = st.radio("Affichage :", ["Total (toutes substances)", "Par substance (empilé)"], horizontal=True)
    ...
    base = df[[key_col, date_col] + ([namex_col] if namex_col else [])].dropna(subset=[key_col, date_col]).copy()
    base[date_col] = _parse_dates_series(base[date_col])
    base = base.dropna(subset=[date_col]).sort_values(date_col).drop_duplicates(subset=[key_col], keep="first")
    ...
    if display_mode.startswith("Total") or not namex_col:
        grouped = base.groupby("period").size().reset_index(name="count").sort_values("period")
        color_enc = None
    else:
        grouped = base.groupby(["period", namex_col]).size().reset_index(name="count").sort_values(["period", namex_col])
        order = (
            grouped.groupby(namex_col)["count"].sum().sort_values(ascending=False).index.astype(str).tolist()
        )
        cmap = _ensure_color_map(order, st.session_state.get("substance_colors", {}), PALETTE)
        st.session_state["substance_colors"] = cmap
        color_enc = alt.Color(
            f"{namex_col}:N", sort=order, legend=alt.Legend(title=namex_col),
    ...
    bar_size = 48 if n <= 15 else 32 if n <= 30 else 20 if n <= 60 else 12 if n <= 120 else 8
    ...
    if display_mode.startswith("Total") or color_enc is None:
        base_chart = alt.Chart(grouped).encode(
            ...
            tooltip=[alt.Tooltip("period:T", title="Période", format=fmt), alt.Tooltip("count:Q", title="Unités uniques")],
        )
        chart = base_chart.mark_bar(size=bar_size, color="#2563EB", cornerRadiusTopLeft=6, cornerRadiusTopRight=6)
        if n <= 60:
            chart = chart + base_chart.mark_text(dy=-6, fontSize=13, color="#111827").encode(text="count:Q")
    else:
        chart = (
            alt.Chart(grouped)
    ...
    agg = st.radio("Agrégation temporelle", ["Tous", "Année", "Mois", "Semaines"], horizontal=True)
    base_p = pure_long[
        (pure_long["substance"] == chosen_substance) & (pure_long["purity_type_display"] == chosen_type)
    ].copy()
    base_p = base_p[["sample_name", "seizure_date", "purity_value", "substance"]].dropna(subset=["purity_value", "sample_name"])
    ...
    if agg == "Tous":
        ...
        def bucket_key(b):
            if agg == "Année":   return pd.Timestamp(int(b), 1, 1)
            if agg == "Mois":
                y, m = b.split("-"); return pd.Timestamp(int(y), int(m), 1)
            if agg == "Semaines":
                y, w = b.split("-W"); return pd.to_datetime(f"{y}-W{w}-1", format="%G-W%V-%u")
    ...
    ymode = st.radio("Échelle Y", ["Auto", "0–100"], index=0, horizontal=True, key="ybox")
    ...
    st.altair_chart(
        ...
        .mark_boxplot(size=30)
        .encode(
            x=alt.X("bucket:N", sort=bucket_order, title=("Période" if agg != "Tous" else "")),
            y=alt.Y("purity_mean:Q", title=f"Pureté (moyenne par sample) — {chosen_type}", scale=yscale),
            tooltip=[alt.Tooltip("bucket:N", title="Période"), alt.Tooltip("purity_mean:Q", title="Pureté (moy. sample)")],
    ...
    recap = (
        ...
        .agg(
            n_samples="count", mean="mean", median="median",
...
# ------------------------- Carte ----------------------------
    ...
    st.caption(f"Colonnes utilisées : latitude = **{lat_col}**, longitude = **{lon_col}**")
    ...
    if date_col:
        ...
        gdf = gdf[(gdf[date_col] >= pd.to_datetime(d1)) & (gdf[date_col] <= pd.to_datetime(d2))]

    if sample_col:
        gdf = gdf.sort_values(date_col if date_col else lat_col).drop_duplicates(subset=[sample_col], keep="first")

    if namex_col:
        options = sorted(gdf[namex_col].dropna().astype(str).unique())
        chosen = st.multiselect("Filtrer par substance (`name_x`)", options=options, default=[])
        if chosen:
    ...
    if cluster_overlay and "assignments" in cluster_overlay and sample_col:
        ...
        if use_clusters:
            ...
            if saved_top_ids and len(saved_top_ids) >= 1 and len(saved_top_ids) == n_top_default:
                top_ids = set(saved_top_ids[: int(n_top_map)])
            else:
                top_ids = (
                    gdf["cluster"].value_counts().sort_values(ascending=False).head(int(n_top_map)).index
                )
                top_ids = set(top_ids)

            gdf = gdf[gdf["cluster"].isin(top_ids)]
            if gdf.empty:
                st.warning(f"Aucun point parmi les {int(n_top_map)} plus grands clusters n’est visible avec les filtres actuels.")
    ...
    if use_clusters and ("cluster" in gdf.columns):
        ...
        if dc and dc in gdf.columns:
            ...
            if gdf["_ts"].notna().any():
                ...
                dmin_ui, dmax_ui = pd.to_datetime(ts_min, unit="s").date(), pd.to_datetime(ts_max, unit="s").date()

                st.markdown("#### Période (glisser pour voir apparaître/disparaître)")
                mode_temps = st.radio("Mode", ["Fenêtre mobile", "Cumul jusqu’à…"], horizontal=True, key="cluster_time_mode")
                if mode_temps == "Fenêtre mobile":
                    d1_sel, d2_sel = st.slider(
                        "Fenêtre temporelle",
                        min_value=dmin_ui, max_value=dmax_ui, value=(dmin_ui, dmax_ui),
                        format="DD/MM/YYYY", key="cluster_time_window",
                    )
                    filter_range = [int(pd.Timestamp(d1_sel).timestamp()), int(pd.Timestamp(d2_sel).timestamp())]
                else:
                    d_sel = st.slider(
                        "Cumul (≤ date)", min_value=dmin_ui, max_value=dmax_ui, value=dmax_ui,
                        format="DD/MM/YYYY", key="cluster_time_cum",
                    )
        ...
        if namex_col:
            ...
            cmap_subs = _ensure_color_map(subs, st.session_state.get("substance_colors", {}), PALETTE)
            gdf["_rgb"] = (
                gdf[namex_col].astype(str).map(cmap_subs).where(lambda s: s.notna(), "#2563EB").apply(lambda c: _hex_to_rgba(c, 180))
            )
        else:
    ...
    view_state = pdk.ViewState(latitude=46.5, longitude=2.5, zoom=6, min_zoom=2, max_zoom=16, bearing=0, pitch=0)

    tooltip_fields = []
    if sample_col: tooltip_fields.append(("Sample", sample_col))
    if namex_col:  tooltip_fields.append(("Substance", namex_col))
    if date_col and (date_col in gdf_view.columns):
        gdf_view["date_str"] = pd.to_datetime(gdf_view[date_col], errors="coerce").dt.strftime("%d/%m/%Y")
        tooltip_fields.append(("Date", "date_str"))
    ...
    layer = pdk.Layer(
        ...
        get_position=[lon_col, lat_col],
        **({
            "get_radius": 400, "radius_min_pixels": 6, "radius_max_pixels": 80,
            "get_fill_color": "_rgb", "pickable": True,
        } if mode == "Points" else {"radiusPixels": 40}),
    )
    st.pydeck_chart(
        deck_with_fallback([layer], view_state, {"html": tooltip_html} if tooltip_html else None),
    ...
    st.download_button(
        ...
        data=gdf_view.drop(columns=["_rgb"], errors="ignore").to_csv(index=False).encode("utf-8"),
        file_name="points_carte.csv", mime="text/csv",
    )
...
# ------------------------ Profilage -------------------------
    ...
    with st.sidebar:
        ...
        cut_range = st.slider("Plage (longueurs d’onde)", int(wl_min), int(wl_max), (default_lo, default_hi))

        c1, c2 = st.columns(2)
        with c1:
            sg_window = st.number_input("Fenêtre SG (impair)", min_value=3, max_value=301, step=2, value=5)
        with c2:
            sg_poly = st.number_input("Ordre poly SG", min_value=2, max_value=5, step=1, value=2)
    ...
    def _replace_spectra(df_in: pd.DataFrame, new_spec: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.concat([df_in[non_spec].reset_index(drop=True), new_spec.reset_index(drop=True)], axis=1)
    ...
    for step in pipeline:
        ...
        if step == "Découpe":
            ...
            keep_cols = [c for c in spec_cols_now if (wls_now[c] >= wl_lo and wls_now[c] <= wl_hi)]
            if not keep_cols:
            ...
            try:
                spec = apply_savgol(spec, window_length=int(sg_window), polyorder=int(sg_poly), deriv=2)
    ...
    if len(spec_cols_final) < 2:
        ...
        if sample_col:
            ...
            s_query = st.text_input("Rechercher un sample_name", placeholder="ex: ABC123")
            filt = [s for s in all_samples if (s_query or "").lower() in s.lower()] if s_query else all_samples
            max_curves = st.slider("Nombre max. de courbes à superposer", 1, 30, 10, 1)
            selected = st.multiselect("Sélectionner un ou plusieurs samples", options=filt, default=filt[:1])

            avg_before_plot = st.checkbox("Moyenner les réplicas par sample avant tracé", value=False)
            if not selected:
                st.info("Choisis au moins un sample pour afficher les courbes.")
            else:
                df_plot = out[out[sample_col].astype(str).isin(selected)].copy()
                if avg_before_plot:
                    df_plot = df_plot[[sample_col] + spec_cols_final].groupby(sample_col, as_index=False).mean(numeric_only=True)
                    df_plot["_series"] = df_plot[sample_col].astype(str)
                else:
                    df_plot["_rep"] = df_plot.groupby(sample_col).cumcount() + 1
                    df_plot["_series"] = df_plot[sample_col].astype(str) + " · r" + df_plot["_rep"].astype(str)

                if len(df_plot) > max_curves:
                    st.warning(f"{len(df_plot)} courbes → affichage limité aux {max_curves} premières.")
                    df_plot = df_plot.head(max_curves)

                long = df_plot[[sample_col, "_series"] + spec_cols_final].melt(
                    id_vars=[sample_col, "_series"], var_name="wl", value_name="intensity"
                )
                long["wl"] = long["wl"].astype(str).str.replace("spectrum_", "", regex=False).astype(float)
                ...
                with st.expander("⬇️ Export des données affichées"):
                    export_df = long.pivot(index=[sample_col, "_series"], columns="wl", values="intensity").reset_index()
                    st.download_button(
            ...
            idxs = st.multiselect("Index de lignes à tracer", options=out.index.tolist(), default=out.index.tolist()[:1])
            if idxs:
                df_plot = out.loc[idxs, spec_cols_final].copy()
                df_plot["label"] = [str(i) for i in idxs]
                long = df_plot.reset_index(drop=True).melt(id_vars=["label"], var_name="wl", value_name="intensity")
                long["wl"] = long["wl"].astype(str).str.replace("spectrum_", "", regex=False).astype(float)
                st.altair_chart(
                    alt.Chart(long)
                    .mark_line()
    ...
    namex_col = _find_col(["name_x", "substance", "substance_name", "drug"], out.columns)
    sample_col = _find_col(["sample_name", "Sample"], out.columns)
    spec_cols_final, _ = _get_spectrum_cols(out)
    ...
    if not namex_col:
        ...
        with col1:
            ...
            target_variants = {"résine thc", "resine thc", "résine de thc", "resine de thc", "thc resin"}
            default_sub_idx = 0
            for i, s in enumerate(subs_options):
                if s.strip().casefold() in target_variants:
                    default_sub_idx = i
                    break
            chosen_substance = st.selectbox("Substance (name_x)", subs_options, index=default_sub_idx, key="dendro_substance")

        with col2:
            methods = ["ward", "average", "complete", "single"]
            link_method = st.selectbox("Méthode de liaison", methods, index=methods.index("complete"), key="dendro_linkage")

        colA, colB = st.columns(2)
        with colA:
            avg_rep = st.checkbox("Moyenner les réplicas par sample", value=bool(sample_col))
        with colB:
            color_threshold = st.number_input("Seuil de distance (0 = auto)", min_value=0.0, step=0.1, value=0.0)

        if date_col_out:
            tmp = out[out[namex_col].astype(str) == str(chosen_substance)].copy()
            if tmp.empty: tmp = out.copy()
            tmp[date_col_out] = _parse_dates_series(tmp[date_col_out])
            min_d = pd.to_datetime(tmp[date_col_out].min());  max_d = pd.to_datetime(tmp[date_col_out].max())
            d1, d2 = st.date_input(
                "Période (filtre appliqué au dendrogramme)",
                value=((min_d.date() if pd.notna(min_d) else datetime(2000, 1, 1).date()),
                       (max_d.date() if pd.notna(max_d) else datetime.today().date())),
            )
        else:
            d1 = d2 = None
        ...
        if date_col_out and d1 and d2:
            ...
            df_sub = df_sub[(df_sub[date_col_out] >= pd.to_datetime(d1)) & (df_sub[date_col_out] <= pd.to_datetime(d2))]
        if df_sub.empty:
        ...
        if avg_rep and sample_col and sample_col in df_sub_valid.columns:
            ...
            labels = (df_sub_valid[sample_col].astype(str).tolist()
        ...
        fig = ff.create_dendrogram(
            X, orientation="bottom", labels=labels, linkagefun=_linkagefun,
        ...
        fig.update_layout(
            ...
            height=620, margin=dict(l=40, r=20, t=60, b=40),
        )
        st.plotly_chart(fig, use_container_width=True, config={"displaylogo": False, "scrollZoom": True})

        # --- Attribution de clusters
        from scipy.cluster.hierarchy import linkage  # (import sûr, déjà dispo)
        Z = _linkagefun(X)

        st.markdown("##### Attribution de clusters")
        cluster_mode = st.radio("Découpe", ["Par nombre de clusters (k)", "Par seuil de distance"], horizontal=True, index=1)
        if cluster_mode == "Par nombre de clusters (k)":
            k = st.slider("k (clusters)", min_value=2, max_value=max(2, min(20, len(labels))), value=4, step=1)
            clusters = fcluster(Z, t=k, criterion="maxclust")
        else:
            thr = st.number_input("Seuil de distance", min_value=0.0, value=float(color_threshold or 0.0), step=0.1)
            clusters = fcluster(Z, t=(thr if thr > 0 else Z[:, 2].mean()), criterion="distance")

        cluster_df = pd.DataFrame({"label": labels, "cluster": clusters.astype(int)})
        sizes = cluster_df["cluster"].value_counts().rename("taille").reset_index().rename(columns={"index": "cluster"})
        cluster_df_full = cluster_df.merge(sizes, on="cluster", how="left")

        tri_mode = st.selectbox("Trier le tableau par…", ["Taille du cluster (décroissante)", "ID de cluster (ascendant)", "Label (ascendant)"])
        if tri_mode.startswith("Taille"):
            cluster_df_sorted = cluster_df_full.sort_values(by=["taille", "cluster", "label"], ascending=[False, True, True])
        elif tri_mode.startswith("ID de cluster"):
            cluster_df_sorted = cluster_df_full.sort_values(by=["cluster", "label"], ascending=[True, True])
        else:
            cluster_df_sorted = cluster_df_full.sort_values(by=["label", "cluster"], ascending=[True, True])

        n_clusters = cluster_df["cluster"].nunique()
        st.markdown(f"**Nombre de clusters : {n_clusters}**  &nbsp;|&nbsp; **Total échantillons : {len(cluster_df)}**")
        st.dataframe(
            cluster_df_sorted.rename(columns={"cluster": "Cluster", "taille": "Taille", "label": "Échantillon"}),
            use_container_width=True, height=320,
        )

        # === Sélection d’un cluster (liste + saisie manuelle) ===
        uniq_ids = sorted(cluster_df["cluster"].unique().tolist())
        csel1, csel2 = st.columns([1, 1])
        with csel1:
            cluster_choice_dropdown = st.selectbox(
                "Choisir un cluster (liste)", options=uniq_ids, index=0, key="cluster_choice_dropdown",
                help="Sélection via la liste déroulante.",
            )
        with csel2:
            cluster_choice_text = st.text_input(
                "…ou saisir l’ID du cluster", value=str(cluster_choice_dropdown), key="cluster_choice_text",
                help="Entre un entier (ex. 3). Si l’ID n’existe pas, la valeur de la liste est conservée.",
            )
            ...
            try:
                ...
                if typed in uniq_ids:
                    ...
                    st.info(f"ID {typed} introuvable parmi les clusters existants : {uniq_ids[:10]}{' …' if len(uniq_ids)>10 else ''}")
            except Exception:
                pass

            st.session_state["cluster_focus_id"] = selected_cluster_id

            focus_preview = cluster_df_full.loc[cluster_df_full["cluster"] == selected_cluster_id]
            with st.expander(f"🔎 Détail rapide — Cluster {selected_cluster_id}"):
                st.dataframe(
                    focus_preview.rename(columns={"label": "Échantillon", "taille": "Taille"}),
                    use_container_width=True, height=260,
                )

        # --- Choix dynamique du nombre de clusters “Top X”
        n_clusters_total = cluster_df["cluster"].nunique()
        n_top = st.number_input(
            "Nombre de plus grands clusters à afficher",
            min_value=1, max_value=int(max(1, n_clusters_total)),
            value=min(10, int(n_clusters_total)), step=1, key="n_top_clusters",
        )

        top_clusters = (
            cluster_df_full.groupby(["cluster"], as_index=False)
            .agg(taille=("label", "count"), exemples=("label", lambda s: ", ".join(s.head(3))))
        ...
        with st.expander(f"🧩 Top {int(n_top)} des clusters les plus grands"):
            st.dataframe(
                top_clusters.rename(columns={"cluster": "Cluster", "taille": "Taille", "exemples": "Exemples"}),
                use_container_width=True, height=300,
            )

        # --- Sauvegarde pour la carte & section détail
        if sample_col:
            cluster_assign = cluster_df[["label", "cluster"]].rename(columns={"label": sample_col}).copy()
            top_ids = (
                cluster_df["cluster"].value_counts().sort_values(ascending=False).head(int(n_top)).index.tolist()
            )
    ...
    def _vm_replace_spectra(df_in: pd.DataFrame, new_spec: pd.DataFrame) -> pd.DataFrame:
        """Remplace les colonnes spectrum_* de df_in par new_spec en conservant les méta-colonnes."""
        non_spec = [c for c in df_in.columns if not (isinstance(c, str) and c.startswith("spectrum_"))]
        return pd.concat([df_in[non_spec].reset_index(drop=True), new_spec.reset_index(drop=True)], axis=1)
    ...
    if vm_pipeline:
        ...
        if not now_cols:
            ...
            # Vérif des fonctions externes si demandées
            if ("SG dérivée 2" in vm_pipeline or "SNV" in vm_pipeline) and (apply_savgol is None or apply_snv is None):
                ...
                for step in vm_pipeline:
                    ...
                    if step == "Découpe":
                        ...
                        vm_out = _vm_replace_spectra(vm_out, spec)
                        st.caption(
                            f"✔️ SG dérivée 2 (fenêtre={st.session_state['vm_sg_window']}, "
                            f"ordre={st.session_state['vm_sg_poly']})."
                        )
    ...
    if vm_namex_col and vm_namex_col in vm_out.columns and vm_namex_col not in vm_work2.columns:
        vm_work2[vm_namex_col] = (
            vm_out[vm_namex_col] if not vm_avg_reps
            else vm_out[["sample_name", vm_namex_col]].drop_duplicates("sample_name")[vm_namex_col].values
        )
    ...
    if vm_date_col and vm_date_col in vm_out.columns and vm_date_col not in vm_work2.columns:
        if vm_avg_reps:
            ...
            meta_dt = (
                ...
                .groupby("sample_name", as_index=False).tail(1)
            )
    ...
    # --- Assemblage des scores + métadonnées
    ...
    vm_scores_df = pd.concat(
        [vm_work2[attach_cols].reset_index(drop=True), pd.DataFrame(vm_scores, columns=comp_names)],
        axis=1,
    ).loc[:, lambda df: ~df.columns.duplicated()]
    ...
    color_enc = (
        ...
        else alt.Color(f"{vm_color_attr}:{'Q' if _is_num(vm_scores_df[vm_color_attr]) else 'N'}", title=vm_color_attr)
    )
    ...
    with c1:
        vm_intra_group_attr = st.selectbox(
            "Grouper (INTRA)",
            vm_group_candidates,
            index=(vm_group_candidates.index("Savonnette") if "Savonnette" in vm_group_candidates else 0),
            key="vm_intra_group",
    ...
    with c2:
        vm_inter_group_attr = st.selectbox(
            "Grouper (INTER)",
            vm_group_candidates,
            index=(vm_group_candidates.index("Lot") if "Lot" in vm_group_candidates else 0),
            key="vm_inter_group",
    ...
    if "sample_name" not in vmD_base.columns and vm_sample_col2 and vm_sample_col2 in vmD_base.columns:
    ...
    # Moyenne par sample si demandé
    def _avg_by_sample(df, group_attr):
        if "sample_name" not in df.columns:
            return df
        gb = ["sample_name", group_attr]
        return df[gb + vm_spec_cols_dist].groupby(gb, as_index=False).mean(numeric_only=True)
    ...
    vm_intra_df = pd.concat(vm_intra_parts, ignore_index=True) if vm_intra_parts else pd.DataFrame(
        columns=["distance", "type", "groupe"]
    )
    ...
    if not vm_both.empty:
        ...
        for lbl, sub in vm_both.groupby("type_label"):
            ...
            pct = cnt / n * 100.0
            rows.append(pd.DataFrame({"bin_label": bin_labels, "bin_center": centers, "pct": pct, "type_label": lbl}))
        ...
        bars = (
            ...
            .mark_bar(size=12)
            .encode(
                x=alt.X("bin_label:N", sort=bin_labels, title="Euclidean Distance (binned)", axis=alt.Axis(labelAngle=0)),
        ...
        rules = alt.Chart(means).mark_rule(strokeDash=[6, 4]).encode(
            x=alt.X("mean_label:N", sort=bin_labels), color=alt.Color("type_label:N", legend=None)
        )
        texts = alt.Chart(means).mark_text(dy=-6).encode(
